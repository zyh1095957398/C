...
#include "stdlib.h"
#define HH printf("%-10s%-10s%-10s%-10s%-10s%-15s%-10s%-15s%-10s\n","学号","姓名","性别","年龄","籍贯","毕业学院","专业班级","电话","毕业方向")
struct graduate             
{
    int id;
    char name[8];
...
void save(struct graduate stu[])
{
	...
	for(i=0;i<n;i++)
	{
		fprintf(fp,"%-10d%-10s%-10s%-10s%-10s%-15s%-10s%-15s%s",stu[i].id,stu[i].name,stu[i].sex,stu[i].year,stu[i].jiguan,stu[i].xueyuan,stu[i].banji,stu[i].phone,stu[i].fx);
	}
    fclose(fp);
}
